<template>
    <div class="content_main">
      <div  style="display:block">
        <div class="left_area">
          <p v-for="(value,index) in classGrade.gradeOne"  :key="index" :data-id="value.gradeId"  :class="{activeCity: changeActivated == index}" @click="chooseActive(index,value.gradeId,value.className)" >{{value.className}}</p>
        </div>
        <div class="right_area">
          <div class="right_area_d on">
            <div class="head_img" :style="{background: `url(/img/headImg/${this.currentId}.jpg)no-repeat center center`}">
              <p @click="chooseClass(currentclass, currentId, 1)" :key="this.currentId" :data-id="this.currentId"> <span></span></p>
            </div>
            <div class="right_third" v-for="(items,index) in classGrade.gradeTwo" :key="index">
              ...
              <div class="third_menu_con" v-show="isActive == index">
                <ul>
                  <li :data-id="items.gradeTwoId" @click="chooseClass(items.twoClass_name, items.gradeTwoId, 2)"></li>
